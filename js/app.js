document.getElementById('submit').addEventListener('click', function() {
  const firstName1 = document.getElementById('firstName1').value.replace(/ /g, '');
  const lastName1 = document.getElementById('lastName1').value.replace(/ /g, '');;
  const firstName2 = document.getElementById('firstName2').value.replace(/ /g, '');;
  const lastName2 = document.getElementById('lastName2').value.replace(/ /g, '');;
  // Validation
  let validationErrorString = '';
  if (firstName1 === '' && lastName1 === '') {
    validationErrorString += 'あなたの名前がありません。\n';
  }
  if ((firstName1.match(/[^a-zA-Z]/)) || (lastName1.match(/[^a-zA-Z]/))) {
    validationErrorString += 'あなたの名前が不正です。アルファベット以外の文字（数字、２バイト文字など）は入れないでください。\n';
  }
  if (firstName2 === '' && lastName2 === '') {
    validationErrorString += '相手の名前がありません。\n';
  }
  if ((firstName2.match(/[^a-zA-Z]/)) || (lastName2.match(/[^a-zA-Z]/))) {
    validationErrorString += '相手の名前が不正です。アルファベット以外の文字（数字、２バイト文字など）は入れないでください。\n';
  }
  if (validationErrorString === '') {
    displayResult(firstName1, lastName1, firstName2, lastName2);
  } else {
    alert(validationErrorString);
  }
});

displayResult = function(f1, l1, f2, l2) {
  const reisuuSelf = getReisuu(f1+l1);
  const reisuuPartner = getReisuu(f2+l2);
  const [seikakuReisuu, seikakuTitle, seikakuDescription] = getSeikaku(reisuuSelf);
  const aisho = getAisho(reisuuSelf, reisuuPartner);
  $('#result').html('<h1>結果</h1>');
  $('<div id="reisuu"></div>')
    .html('<p>あなた (' + f1 + ' ' + l1 + ') の霊数：' + reisuuSelf + '<br>' + '相手 (' + f2 + ' ' + l2 + ') の霊数：' + reisuuPartner + '</p>')
    .appendTo('#result');
  $('<div id="seikaku"></div>')
    .html('<h2>名前の霊数が教える基本的なあなたの性格</h2>')
    .html('<h3>霊数' + reisuuSelf + '：' + seikakuTitle + '</h3><p>' + seikakuDescription + '</p></div>')
    .appendTo('#result');
  $('<div id="aisho"><h2>あなたと相手の名前の霊数の相性</h2>')
    .append('<h3>' + reisuuSelf + ' ←→ ' + reisuuPartner + '</h3>')
    .append('<p>' + aisho + '</p></div></div>')
    .appendTo('#result');
}

// displayAbout = function() {
//   var p = [];
//   p.push('私たちが普段何気なく使っている文字。言葉に“ことだま”が宿るといわれたのと同様に、文字にも霊性が宿るという説は、古今東西、いわれ続けた事実です。日本では、中国から伝わった漢字の画数占いが有名ですが、西洋でも同様にアルファベットによる数字占いがさかんに行われてきました。これはＧＥＭＡＴＲＩＡ《ゲマトリア》占数術と呼ばれる占い。では、どうして人々は、アルファベットにそのような霊性があると考えるようになったのでしょうか。');
//   p.push('地球上の人間はどんな未開の地の民族でも、必ず言葉をしゃべっています。しかし、その民族がすべて文字を持っているかというと、必ずしもそうではありません。');
//   p.push('文字の歴史をたどってみると、その民族の文字は必ずといっていいほど外国人によってもたらされて来たことがわかります。日本の文字が中国の漢字の変形であることは誰もが知っていますが、その中国の漢字ですら、さらに異国からの伝来物であるというのです。');
//   p.push('文明の発祥地であるシュメールでは、今から約５０００年前に人々に文字が伝わったといわれています。そして、人々に文字を広めたのはオアンネスと呼ばれる高度な知識者だったといわれています。が、そのオアンネスは決して人々と食事を共にせず、夜になると海の中へ戻って行ったーーシュメールにはそんな神話が残されています。');
//   p.push('一方ギリシャではフェニキアの王子カドモスが持ち込んだアルファベットが、紀元前７７６年に開催された第１回の古代オリンピック大会に使用されたことが文献に残されています。そのフェニキアのアルファベットも元をたどると、ある船乗りが西方である人物から教わったといわれています。さて、その人物とは一体誰だったのでしょうか？');
//   p.push('このように、文字、アルファベットの出現の影には実に外国人や神らしき人物の影がちらついています。そのため、文字、特にアルファベットは事によると神が人間に遣わした暗号かも知れない、という考え方が生まれてきたのです。');
//   p.push('そこで古代ヘブライのギリシャ、およびローマでは、ある言葉のアルファベット表記をそのアルファベットに当てられた一定の数値に置き換えてその数字に隠されている意味を読解しようという占数術が生まれてきました。これが、ＧＥＭＡＴＲＩＡ《ゲマトリア》占数術なのです。');
//   p.push('例えば、聖書の黙示録に出てくる「獣の数、６６６」も、このＧＥＭＡＴＲＩＡ《ゲマトリア》占数術によって解読されたもので、実はキリスト教を弾圧した暴君ネロを指しているということが後世になって分かってきました。');
//   p.push('さらに、楽園を意味するエデン《ＥＤＥＮ》やなぞの大陸アトランティス《ＡＴＬＡＮＴＩＳ》、《Ｕ・Ｆ・Ｏ》、それにユートピア《ＵＴＯＰＩＡ》などが揃って霊数０であることが分かり、まさに、「存在するかしないかが分からない数＝０」になることが検証されたのです。');
//   p.push('このＧＥＭＡＴＲＩＡ《ゲマトリア》占数術を活用すれば、あなたの名前に隠された性格はもちろん、あなたと相手の相性までがはっきりと分かってしまうのです。');
//   p.push('このサイトを使って、あなたと相手の名前に隠された霊数を割り出し、シュメールの太古から伝わるＧＥＭＡＴＲＩＡ《ゲマトリア》占数術で、２人の性格とこれからの運命を占ってみてください。');
//    $('<h1>ＧＥＭＡＴＲＩＡ《ゲマトリア》とは？</h1>').appendTo('#content');
...
//   }
//   $('<div class="lead"></div>')
//     .append('<p style="text-align: right">岡野タケシ氏（アート・コンサルタント）より</p>')
//     .appendTo('#content');
// }
...
function getSeikaku(reisuu) {
  let seikakuValues = ['', '', ''];
  const seikakuRow = [
    '0,本心を見せない２面性の持ち主,明るいけれど、なかなか本心を見せない人です。０には生と死の二面性があり、楽天と絶望が交互に表れます。無欲無心で超然としているように見えても、突然、爆発的なエネルギーを発揮して周りの人を驚かせることがあります。恋をしても一途に打ち込むことはなく、また徹底的にフラれても不死鳥のように再生します。', 
    '1,プライドの高さが独裁的に変貌,尊大で誇り高き王者のよう。「我こそ、ナンバー１」と思っている人です。創造力や指導力を発揮して人々の信頼を集める面もありますが、反面、自己主張が過ぎて独断的になり、さらに口うるさくなったり怒りっぽくなって嫌われることも。恋には限りなく所有欲の強さが表れますからＳＥＸ面は自制心が強い方です。', 
    '2,シニカルで高い協調性の持ち主,人当たりはソフト。しかしなかなかの皮肉屋で「ああいえばこういう」反対意見の持ち主です。２が暗示するものは「分裂」「対立」「葛藤」で、秩序に反抗的。しかし「月」「夜」に象徴されるように「秘密」を大切にし、不思議と協調性を生み出します。また熱中・陶酔派で、恋は純潔性、処女性を重視。ＳＥＸはエンジョイ型です。', 
    '3,夢見がちな傷つきやすい善人,保守的で良心的。すぐに「道徳」や「戒律」を口にするかと思うと「愛」「平和」を説いてまわるカルチャー型の人です。さらに最も夢多き人であれもしたい、これもしたいと計画を立てます。集中力もあり冒険心も大。そのために犠牲を受けることも多いでしょう。恋は「自分なしでは相手はダメになる」と片思いの傾向があります。', 
    '4,秩序を保った高バランス人間,流行に敏感。多様な生き方ができる高バランス感覚人間。４は４本の柱とか東西南北のように「秩序ある配置」を暗示し、柔軟な対応力を持つことを示します。また、商才を大いに発揮し、正義感や友情にも厚い人です。落とし穴は直感に頼り過ぎて、失敗を招くこと。恋は純潔志向。セックスはコミュニケーションを重視します。', 
    '5,恋は盲目！ 楽天的な理想論者,少々空想的・楽天的部分の多い人です。５は星型を現し、この持ち主は特に「夢」「希望」「理想」を大切にします。また、論理を組立てるのが大好き。しかしこの理論も空想の部分が大きすぎて空中分解することがあります。でも、復原力は早く、バランスを保つのが上手。意見の衝突を起こしやすいのが欠点。恋には盲目です。', 
    '6,几帳面で努力家の完全主義者,神経質なほどの完璧主義者です。さらに、一方を見ると、他が見えなくなる傾向があります。よくいえば誠実ですが、悪くいえば強情。でも努力と几帳面さがモットーです。行動力と積極性を合わせ持っているためピンチにも強く、極めて野心的ですが、利己的で支配欲も強い。恋は感情過多で自分がリードしたがります。', 
    '7,最も大切なのは精神的な成長,７は神の「恵み」や「メッセンジャー」を現す数。「７つの封印」「７つの霊」「７つの星」「７つの燈台」など聖書にはおびただしい数の７が登場し、精神を誘導する一面を暗示しています。生活面では「親切」がモットー。反面プライドと野心も大。また、暴力性を表すことも。恋愛はデリケートな一体感を求める傾向にあります。', 
    '8,自由奔放。明るくノンキな情熱家,８は∞（無限大）に通じ、永遠願望の象徴です。人生は何回でもやり直しがきく、とノンキに考える人。モットーは情熱と繁栄。明るく、にぎやかであることを好みますが、反面、ワザと騒ぎを起こしたり、ライバルを作って競いたがる傾向もあり、異性を誘惑するなどのスキャンダラスな面も。自由奔放な恋を好みます。', 
    '9,永遠の心理を追求する哲学者,神秘的なかげりが人目を魅きつけます。社交性があり、華麗な面が目立ちますが、どこかに不安定で自己充足に追われている様子が伺えます。もともと９は完全な男性の数で、「真理の追求」を指す数字。破滅や返送も示しますが、同時に「再生」と「思いがけぬ幸運」も表します。恋は精神的なものを求める傾向にあります。'
  ];
  seikakuValues = seikakuRow[reisuu].split(',');
  return seikakuValues;
}
...
function getAisho(r1, r2) {
  ...
  msg.push('どちらも男性的な気質マル出しで、ぶつかり合うライバル関係の２人です。しかしあくまでも競い合うのであって、悪くてもケンカや絶交というところまでは発展しません。ただお互いに意地の張り合いがあって一度こじれると自分から折れることができません。できれば別々の仕事を持って、時々会うようにしたほうがベターです。');
